...
- Name: Creating UG Pages >> Create a build page on Marvel Rivals project
- Location: /home/runner/work/qa-automated-tests/qa-automated-tests/tests/ngf.test.js:262:7
...
Call log:
  - waiting for getByRole('button', { name: 'Create' })
    ...
    at /home/runner/work/qa-automated-tests/qa-automated-tests/tests/ngf.test.js:272:30
```
...
```ts
  ...
  11 |       .getByRole("button")
  12 |       .filter({ hasText: /^$/ });
  13 |     this.buildWidget = (build) =>
  14 |       page.getByRole("main").filter({ hasText: `${build}` });
...
> 28 |       await this.createButton.click();
  ...
  44 |     });
  45 |   }
  46 |   async deleteBuild(pageId) {
  47 |     await test.step(`Delete ${pageId} build page`, async () => {
